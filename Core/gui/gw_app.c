...
static uint8_t prv_get_beacon_burst_count(void)
{
    /* 최신 요구:
     * - 주기 beacon은 1회만 송신
     * - 설정 변경/수동 beacon도 1회만 송신
     * 같은 beacon을 반복 송신하지 않는다. */
    return 1u;
}
...
{
    /* 최신 요구: GW NUM(0/1/2)에 따라 beacon phase를 0/2/4초로 맞춘다.
     * 1M/2M 테스트 모드도 beacon 시각 자체는 GW NUM을 따른다.
     * RX/TCP 기준 시각은 기존 요구(30초/40초/90초)를 유지한다. */
...
static bool prv_get_reminder_offset_sec(uint32_t* out_offset_sec)
{
    ...
    /* 최신 요구:
     * - beacon은 한 번만 송신
     * - reminder/retry beacon은 사용하지 않음 */
    return false;
}
    ...
    if ((ev & GW_EVT_WAKEUP) != 0u)
    {
        ...
        if (catm1_due_now)
        {
            ...
            if (s_last_catm1_slot_id != slot_id)
            {
                ...
                if (prv_is_two_minute_mode_active())
                {
                    /* SETTING:2M은 00초 beacon / 30초 RX / 01분 30초 TCP가 고정 요구사항이므로,
