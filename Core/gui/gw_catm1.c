...
#endif
#ifndef GW_CATM1_BOOT_ARM_GUARD_MS
#define GW_CATM1_BOOT_ARM_GUARD_MS    (100u)
#endif
#ifndef GW_CATM1_BOOT_NO_RX_ABORT_MS
#define GW_CATM1_BOOT_NO_RX_ABORT_MS  (2500u)
#endif
#ifndef GW_CATM1_BOOT_SMS_READY_QUIET_ABORT_MS
#define GW_CATM1_BOOT_SMS_READY_QUIET_ABORT_MS (1500u)
#endif
...
static bool prv_wait_boot_sms_ready(void)
{
    ...
    /* power-on 직후에는 마지막 "SMS Ready"를 모뎀 부팅 완료 기준으로 사용한다.
     * 최신 요구:
     *  - SMS Ready가 안 오면 즉시 그 세션을 접는다.
     *  - boot chatter가 끝났는데 SMS Ready가 없으면 전체 timeout까지 질질 끌지 않는다. */
    while ((uint32_t)(HAL_GetTick() - start) < GW_CATM1_BOOT_URC_WAIT_MS)
    {
        if (!prv_catm1_rb_pop_wait(&ch, 20u))
        {
            uint32_t elapsed = (uint32_t)(HAL_GetTick() - start);
            uint32_t idle_ms = (uint32_t)(HAL_GetTick() - last_rx);

            if ((!saw_any_rx) && (elapsed >= GW_CATM1_BOOT_NO_RX_ABORT_MS))
            {
                return false;
            }

            if (saw_any_rx && (idle_ms >= GW_CATM1_BOOT_SMS_READY_QUIET_ABORT_MS))
            {
                return false;
            }
            continue;
...
static bool prv_start_session(bool enable_time_auto_update)
{
    ...
    GW_Catm1_PowerOn();
    /* SIM7080의 "SMS Ready" URC는 power-on 직후 짧게 지나갈 수 있으므로,
     * 긴 고정 boot wait를 두지 않고 아주 짧은 guard 후 바로 URC 수신을 감시한다. */
    prv_delay_ms(GW_CATM1_BOOT_ARM_GUARD_MS);

    /* 새 session 시작: 아직 AT OK를 못 받음 */
    s_catm1_session_at_ok = false;

    /* 요구사항: power-on 시 "SMS Ready"가 안 오면 더 진행하지 않고 바로 종료한다. */
...
static size_t prv_build_snapshot_payload(const GW_HourRec_t* rec, char* out, size_t out_sz)
{
    ...
    last_nd_num = (cfg->max_nodes > 0u) ? (uint8_t)(cfg->max_nodes - 1u) : 0u;
...
void GW_Catm1_PowerOff(void)
{
    ...
    /* 요구사항 유지:
     * - 현재 session에서 AT OK를 한 번도 못 받았으면 긴 종료 절차를 밟지 않는다.
     * - SMS Ready 미도달 / AT sync 실패 세션은 빠른 hard power-off로 끝낸다. */
